...
from GDINO_detector import GDINO_detector
import dataset
...
def yolo_sma2_run(video_dir,video_name,buffer_name):
        ...
        for idx,frame in enumerate(frame_names):
            ...
            if len(frame_buffer_list)==frame_buffer_size or idx==len(frame_names)-1:
                ...
                #
                # f_idx=int(os.path.basename(f_path).split(".")[0])
                # execute yolo detection
                result = GDINO_detector(f_path)
                boxs = torch.tensor(result[0]).cpu()  # transform data from gpu to cpu in form of (x1,y1,x2,y2)
                b_conf=torch.tensor(result[1]).cpu() # get the confidence of each box
                ...
                # add new boxs if exists
                    ...
                    # non-first section do the match
                        ...
                        # track_objs_count = len(inference_state["obj_ids"])
                ...
                if len(inference_state["obj_ids"])>0 and not initial_empty_tag:
                    ...
                    # print("max video segments frame is: ", max(video_segments.keys()))
